package dream.cameracalibration;
...
public class CameraCalibrator {
    ...
    private final Size mPatternSize2 = new Size(6, 9);//new Size(4, 11);
    private final int mCornersSize = (int)(mPatternSize.width * mPatternSize.height);
    private final int mCornersSize2 = (int)(mPatternSize2.width * mPatternSize2.height);
    ...
    private void calcBoardCornerPositions(Mat corners) {
        final int cn = 3;
        if (isChessboard){
            float positions[] = new float[mCornersSize2 * cn];

            for (int i = 0; i < mPatternSize2.height; i++) {
                for (int j = 0; j < mPatternSize2.width * cn; j += cn) {
                    positions[(int) (i * mPatternSize2.width * cn + j + 0)] =
                            (2 * (j / cn) + i % 2) * (float) mSquareSize;
                    positions[(int) (i * mPatternSize2.width * cn + j + 1)] =
                            i * (float) mSquareSize;
                    positions[(int) (i * mPatternSize2.width * cn + j + 2)] = 0;
                }
            }
            corners.create(mCornersSize2, 1, CvType.CV_32FC3);
    ...
    private void drawPoints(Mat rgbaFrame) {
        if (isChessboard){
            Calib3d.drawChessboardCorners(rgbaFrame, mPatternSize2, mCorners, mPatternWasFound);
        }else {
